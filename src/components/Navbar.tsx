...
import { Bird, LogIn, UserPlus, User, Settings, LogOut, Menu, X, Sun, Moon, Trophy, ListChecks, Calendar } from 'lucide-react'
      ...
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-2">
            <Link
              to="/workshops"
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 ${location.pathname === '/workshops'
            ...
            <Link
              to="/leaderboard"
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 ${location.pathname === '/leaderboard'
                ? 'text-white bg-gradient-to-r from-primary-600 to-primary-500 shadow-md shadow-primary-500/30'
                : 'text-slate-600 dark:text-slate-300 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-slate-100 dark:hover:bg-dark-surface/50'
                }`}
            >
            ...
            <Link
              to="/checklist"
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 ${location.pathname === '/checklist'
                ? 'text-white bg-gradient-to-r from-primary-600 to-primary-500 shadow-md shadow-primary-500/30'
                : 'text-slate-600 dark:text-slate-300 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-slate-100 dark:hover:bg-dark-surface/50'
                }`}
            >
            ...
            {user && isAdmin && (
              <Link
                to="/admin"
                className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 ${location.pathname === '/admin'
                  ? 'text-white bg-gradient-to-r from-primary-600 to-primary-500 shadow-md shadow-primary-500/30'
                  : 'text-slate-600 dark:text-slate-300 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-slate-100 dark:hover:bg-dark-surface/50'
                  }`}
              >
          ...
          <div className="hidden md:flex items-center space-x-4 pl-4 border-l border-slate-200 dark:border-dark-border">
            {user ? (
              <div className="flex items-center space-x-4">
                <Link to="/profile" className="flex w-full space-x-2 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-dark-surface px-4 py-2 rounded-xl hover:bg-slate-200 dark:hover:bg-dark-border transition-colors">
                  <User className="h-4 w-4" /><span>Welcome,</span> <span className="text-primary-600 dark:text-primary-400">{username || user.email}</span>
                </Link>
                <button
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden glass rounded-xl mt-2 mb-4 overflow-hidden shadow-xl border border-slate-200 dark:border-dark-border">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              <Link
                to="/workshops"
                className={`flex items-center space-x-2 px-3 py-3 rounded-lg text-sm font-medium transition-colors ${location.pathname === '/workshops'
              ...
              {user ? (
                <div className="px-3 py-4 border-t border-slate-200 dark:border-dark-border mt-2">
                  <Link to="/profile" onClick={() => setIsMobileMenuOpen(false)} className="flex w-full space-x-2 py-2.5 text-sm text-slate-600 dark:text-slate-400 mb-3 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                    <User className="h-5 w-5" />
                    <span>Welcome,</span><span className="font-medium text-slate-900 dark:text-white">{username || user.email}</span>
                  </Link>
                  <button
                    onClick={() => {
                      handleLogout()
